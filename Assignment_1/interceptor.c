...
struct pid_list {
	pid_t pid;
	struct list_head list;     //which has a *prev and a *next
};
...
typedef struct {
	...
	asmlinkage long (*f)(struct pt_regs);    //f is a pointer which point to a function which has struct pt_regs as parament and return long
...
spinlock_t pidlist_lock = SPIN_LOCK_UNLOCKED;           //spin_lock(&pidlist_lock)   spin_unlock(&pidlist_lock)
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    int sysc;
    sysc=reg.ax;
    if(table[sysc].intercepted==1)
    {
        //monitor=1&& current pid is monitored
        //monitor=2&& current pid is not monitored which means that this pid is monitored
        if((table[sysc].monitored==1&&check_pid_monitored(sysc,current->pid))||(table[sysc].monitored==2&&!check_pid_monitored(sysc,current->pid)))
        {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    //check for -EINVAL
    if((cmd!=REQUEST_START_MONITORING&&cmd!=REQUEST_STOP_MONITORING&&cmd!=REQUEST_SYSCALL_INTERCEPT&&cmd!=REQUEST_SYSCALL_RELEASE)==1||syscall<0||syscall>NR_syscalls||syscall==MY_CUSTOM_SYSCALL)
        return -EINVAL;
    ...
    if(cmd==REQUEST_SYSCALL_INTERCEPT)
    {
        ...
        if(current_uid()!=0)
            return -EPERM;
        //system call is already intercepted
        ...
        //synchronization and replace the original system call with interceptor
    ...
    }else if(cmd==REQUEST_SYSCALL_RELEASE)
    {
        ...
        if(current_uid()!=0)
            return -EPERM;
        //system call is not intercepted
        if(table[syscall].intercepted==0)
            return -EINVAL;
        //synchronization and restore the original system call
        spin_lock(&calltable_lock);
        spin_lock(&pidlist_lock);
        set_addr_rw((unsigned long)sys_call_table);
    ...
    }else if(cmd==REQUEST_START_MONITORING)
    {
        ...
        //if monitored=2 which means we have to monitor all pids except some pids we stop monitoring
        if(table[syscall].monitored==2)
        {
            if(check_pid_monitored(syscall,pid)==0&&pid!=0)
                return -EBUSY;
            //if pid = 0 , reset to monitor all pids
            if(pid==0)
            {
                spin_lock(&pidlist_lock);
//                add_pid_sysc(pid, syscall);
            ...
            //if pid != 0 which means to add pid which has already be stopped
            else
            {
                spin_lock(&pidlist_lock);
        ...
        //if monitor = 1 or 0
            ...
            //check whether system call is already monitored
    ...
    }else if(cmd==REQUEST_STOP_MONITORING)
    {
        ...
        //check whether system call is intercepted
        if(table[syscall].intercepted==0)
            return -EINVAL;
        //check pid is already stopped monitoring
        else if(table[syscall].monitored==2)
        {
            ...
            return -EINVAL;
        //if pid = 0 , we stop monitoring all pids
        if(pid==0)
        {
            if(table[syscall].monitored==0)
                return -EINVAL;
        ...
        //if pid!=0 and monitored = 2 , we remove pid from the monitoring list
        else if(table[syscall].monitored==2)
        {
            spin_lock(&pidlist_lock);
            add_pid_sysc(pid,syscall);
        ...
        //if pid!=0 and monitored =1 ,we remove pid from the monitoring list
        else
        {
            spin_lock(&pidlist_lock);
            del_pid_sysc(pid,syscall);
